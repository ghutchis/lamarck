...
def zoomin():
    ...
    recalc = False # The first time you run this you need to use recalc = True
    alleffs = {2:[], 4:[]}
    if not recalc:
...
##    shorten = True
##    if shorten:
##        alleffs[2] = alleffs[2][:1000]
##        alleffs[4] = alleffs[4][:1000]
##        json.dump(alleffs, open("tmp2.json", "w"))
    mpl.rc('font', size=16)

    plothist = False
    if plothist:
        for length in [2, 4]:
            ax = pylab.subplot(120 + length/2)
            if recalc:
            ...
            pylab.ylim(0, 2750)
            pylab.title(dbname[length][3:-1].capitalize())
            pylab.xlabel("Efficiency (%)")
            if length == 2:
                pylab.ylabel("Counts")

            ax.annotate("%d" % a[0], xy=(0.3, 2580),  xycoords='data',
            ...
            if length == 4:
                subax = pylab.axes([.80, .28, .14, .4])
                a, b, c = pylab.hist(alleffs[length], facecolor="gray", bins=np.arange(0.0, 12, 0.25))
                pylab.setp(subax, xlim=(6.25, 11), ylim=(0, 250))
        ...
        pylab.subplots_adjust(bottom=0.13, top=0.93, right=0.97, left=0.11)
...
##                   [q[3] for q in selected], "r" + dot)
        ...
        if N < 2:
            pylab.title({2:"Dimers", 4:"Tetramers"}[length])
...
##            t.set_fontsize(9)
    ...
    if not supplementary:
        pylab.savefig(os.path.join("pictures", "Figure2_dims_and_tets.png"))
    else:
##        pylab.show()
        pylab.savefig(os.path.join("pictures", "Supplem_Figure2_dims_and_tets.png"))
